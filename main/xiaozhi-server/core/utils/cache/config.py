...
class CacheConfig:
    ...
    max_size: Optional[int] = 200  # 低配服务器默认200条
    ...
    def for_type(cls, cache_type: CacheType) -> "CacheConfig":
        ...
        configs = {
            CacheType.LOCATION: cls(
                strategy=CacheStrategy.TTL, ttl=None, max_size=200  # 手动失效
            ),
            CacheType.IP_INFO: cls(
                strategy=CacheStrategy.TTL, ttl=86400, max_size=200  # 24小时
            ),
            CacheType.WEATHER: cls(
                strategy=CacheStrategy.TTL, ttl=28800, max_size=200  # 8小时
            ),
            CacheType.LUNAR: cls(
                strategy=CacheStrategy.TTL, ttl=2592000, max_size=365  # 30天过期
            ),
            CacheType.INTENT: cls(
                strategy=CacheStrategy.TTL_LRU, ttl=600, max_size=200  # 10分钟
            ),
